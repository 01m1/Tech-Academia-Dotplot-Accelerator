        ...
        if (data.tumour_image == null) {
          ...
          if (Array.isArray(data.tumour_image)) {
            ...
            while (i < data.tumour_image.length) {
              const timestamp = new Date().getTime();
              realTumour.push([
                `http://127.0.0.1:8000${data.tumour_image[i]}?t=${timestamp}`,
              ]);
              i++;
            }
          } else {
            const timestamp = new Date().getTime();
            realTumour = [
              `http://127.0.0.1:8000${data.tumour_image}?t=${timestamp}`,
            ];
          }
        }
  ...
  return (
    <div>
      <div className="w-[270px] mx-4 rounded h-[auto]   py-10 ">
        ...
        <div className="border shadow px-8 py-10 rounded-md text-[15px] bg-[#3E737A] ">
          {!id ? (
            <h1 className="p-5 font-bold rounded text-white text-l text-center">
              Patient Not Found
            </h1>
          ) : (
